...
class BalanceLib {
	...
	async totalUserBalance(currentMonthFirstDate, currentMonthLastDate, id){
		try {
	...
	async categoryWiseBalance(currentMonthFirstDate, currentMonthLastDate,balanceCatId){
		try {
			const data = await BalanceModel.find({
				balanceCategoryId: balanceCatId,
				date: {
					$gte:  currentMonthFirstDate,
					$lte:  currentMonthLastDate,
				}
