...
class UserLib extends ExpenseLib {
	...
	async userSummary(currentMonthFirstDate, currentMonthLastDate, userId){
		try {
			// user total  meal balance
			const totalSavings = await BalanceLib.userMealBalance(currentMonthFirstDate, currentMonthLastDate, userId)
			// user total meals
			const meal = await MealLib.userWiseMeal(currentMonthFirstDate, currentMonthLastDate, userId);
			// meal rate
			// return  (meal rate * total meal) - user total meal balance
			return {
